...
void scheduler(void)
{
  for (;;)
  {
    ...
    // The default scheduling algorithm is implemented here
    ...
    // The Quantum scheduling algorithm is executed here
    ...
    // The priority algorithm is implemented here
...
//This system call prints my favorite year .
...
{
  struct proc *p;
  //int currentQuantum=ptable.proc->Quantum + 10;
  //cprintf("Hi");

  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    // p->Quantum=currentQuantum;
  }
  return 2;
}
...
//This function initializes the amount of Quantum variable to 10 .
...
// This function updates the time variables foe each process
void waitForChiled(int ticks)

{
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
  {
    ...
    if (p->state == RUNNING)
    {
      ...
      ++p->runningTime;
      p->changeablePriority = p->changeablePriority + p->priority;
    }
    if (p->state == SLEEPING)
    {
      ++p->sleepTime;
